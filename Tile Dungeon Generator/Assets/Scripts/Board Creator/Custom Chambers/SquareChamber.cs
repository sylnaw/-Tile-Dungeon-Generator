...
public class RectangularChamber : RoomPattern
{
    ...
    public override TileVector GetVectorForNewRoom(Direction direcionOfNewRoom, Room chamber, RoomType typeOfNewRoom)
    {
    ...
    Vector2Int GetVectorPosition(Direction direction, Room chamber)
    {
        if (direction.IsNorth || direction.IsSouth)
        {
            ...
            if (direction.IsNorth) return new Vector2Int(randomX, chamber.endPosition.y + 1);
            else return new Vector2Int(randomX, chamber.startPosition.y - 1);
        }
        else
        {
            ...
            else return new Vector2Int(chamber.startPosition.x - 1, randomY);
        }
    }
